  ...
  <script>
    ...
    $(document).ready(function() {
      $('#dataTable').DataTable({
        ...
        serverSide : true,
        ajax : {
          url: "{{ route('jurusan.get_datatable') }}",
        //   type: 'GET'
        },
        columns: [
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" id="data_id">
          <div class="form-group">
              <label for="name_edit" class="control-label">Jurusan</label>
          ...
          <div class="form-group">
              ...
              <input type="number" class="form-control" id="total_edit" name="total_edit" placeholder="">
          </div>
      </div>
      <div class="modal-footer">
          <button type="button" class="btn btn-danger" data-dismiss="modal"><i class="fa fa-x"></i></button>
  ...
  <script>
    $('body').on('click', '#btn-edit', function () {
      let data_id = $(this).data('id');
        $.ajax({
          url: `{{ url('admin/jurusan/show/${data_id}') }}`,
          type: "GET",
          cache: false,
          success: function (response) {
    ...
    $('#update').click(function (e) { 
        ...
        let total   = $('#total_edit').val();
        let token   = $("meta[name='csrf-token']").attr("content");
  
        $.ajax({
            url: `{{ url('admin/jurusan/update/${data_id}') }}`,
            type: "POST",
            cache: false,
            data: {
  ...
  <script>
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          url: `{{ url('admin/jurusan/destroy/${data_id}') }}`,
          type: "DELETE",
          cache: false,
          data: {
            "_token": token
